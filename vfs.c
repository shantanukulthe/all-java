...
Marvellous Infosystems : Logic Building & Industrial Project Development
...
int WriteFile(int fd, char *arr, int isize)//(0,abcd,4)
{
...
int CloseFileByName1(char *name)//done
{
	int i = 0;
	...
	UFDTArr[i].ptrfiletable->readoffset = 0;
	UFDTArr[i].ptrfiletable->writeoffset = 0;
	(UFDTArr[i].ptrfiletable->ptrinode->ReferenceCount)--;//refrence become 0 which says file is close
...
int LseekFile(int fd, int size, int from)//(0,3,1)
{
...
int main()
{
	...
	while(1)
	{
		...
		{
			if(strcmp(command[0],"stat") == 0)//stat demo.text
			{
			...
			else if(strcmp(command[0],"close") == 0)//close Demo.text
			{
			...
			else if(strcmp(command[0],"rm") == 0)//rm demo.text//remove the command
			{
			...
			else if(strcmp(command[0],"write") == 0)//write demo.text enter 
			{
				...
				ret = WriteFile(fd,arr,ret);// (0,arr base address,4)
				if(ret == -1)
				...
				if(ret==-4)
				{
					printf("ERROR : There is no suffient memory avilable\n");
				}
				if(ret>0)
				{
					printf("SCUSSE :%d byte sucesssfull return",ret);
				}
			}
